describe("Test Loading Sequence and Functions", () => {

    jasmine.DEFAULT_TIMEOUT_INTERVAL = 10000; //loading takes longer than 5000ms

    var chooseGameSpy;

    beforeAll((done) => {
        chooseGameSpy = spyOn(window, 'chooseGame');
        loadGame();
        setTimeout(() => {
            done();
        }, 6000);
    ...
    it("Should reach the end of the loading sequence", () => {
        expect(window.chooseGame).toHaveBeenCalled();
    })
    ...
    describe("Test data gained from the loading sequence", () => {
